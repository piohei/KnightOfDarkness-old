...
class ShoutBox {
	...
	private function checkFlooding(){
		$mmm = $this->_dbd->GetMessages(5);
		$lastNick = 'aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa';
		$lastNickCount=1;
		foreach ($mmm as $i => $mm) {
			if (strcmp($mmm[$i]['NICK'],$lastNick)==0){
				$lastNickCount++;
			} else {
				$lastNickCount=1;
				$lastNick = $mmm[$i]['NICK'];
		...
		if (strcmp($lastNick,$this->_nick)==0){
			if ($lastNickCount > 3){
				$this->userMode |= self::F_WARNED;
			} elseif ($lastNickCount > 4){
	...
	public function GenerateXML(){
		...
		foreach ($this->_messages as $_message){
			...
			$ret.= '<message id="'.$_message['ID'].'" owner="'.(!empty($_message['ADMIN']) ? 2 : ((empty($_message['ID_USER']) && empty($_message['NICK'])) ? 3 : (($_message['ID_USER'] && $_message['ID_USER'] == $this->_idUser) ? 1 : 0))).'"'.($_canDel ? ' candelete="1"' :'').($_canEdit ? ' canedit="1"' :'').'>'.
